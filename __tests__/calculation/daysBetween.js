...
it('returns 0 when given two objects representing the same day', t => {
	t.is(daysBetween(one, one), 0)
	t.is(daysBetween(other, other), 0)
})

it('return a positive integer if the second argument is later than the first', t => {
	t.is(daysBetween(one, other), 5)
	t.is(daysBetween(one, otherYear), 250)
})

it('return a negative integer if the second argument is earlier than the first', t => {
